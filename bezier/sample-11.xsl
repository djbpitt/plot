...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="djb:bezier-f" as="element(svg:g)">
        <!-- ================================================================= -->
        <!-- Function parameters                                               -->
        <!-- ================================================================= -->
        ...
        <!-- Fail on bad $scaling                                              -->
        <!-- ================================================================= -->
        <xsl:if test="$scaling lt 0 or $scaling gt 1">
            <xsl:message terminate="yes" select="'$scaling argument is invalid: ' || $scaling"/>
        </xsl:if>
        <!-- ================================================================= -->
        ...
        <!-- Function variables                                                -->
        <!-- ================================================================= -->
        <!-- $pointPairs as xs:string+ : tokenize $points on whitespace        -->
        ...
        <!-- ================================================================= -->
        <xsl:variable name="pointPairs" as="xs:string" select="tokenize($points, '\s+')"/>
        <xsl:variable name="xPoints" as="xs:double+"
            select="$pointPairs ! substring-before(., ',') ! xs:double()"/>
        <xsl:variable name="yPoints" as="xs:double+"
            select="$pointPairs ! substring-after(., ',') ! xs:double()"/>
        <xsl:variable name="points" as="element(point)+">
            <xsl:for-each select="1 to count($xPoints)">
                <xsl:element name="point" xmlns="">
                    <xsl:value-of
                        select="string-join(($xPoints[current()], $yPoints[current()]), ',')"/>
                </xsl:element>
            </xsl:for-each>
        </xsl:variable>
        ...
        <xsl:variable name="dirXs" as="xs:integer+">
            <xsl:for-each select="1 to count($points) - 2">
                ...
                <xsl:variable name="x2" as="xs:integer"
                    select="$xPoints[position() eq current() + 2]"/>
        ...
        <xsl:variable name="dirYs" as="xs:integer+">
            <xsl:for-each select="1 to count($points) - 2">
                ...
                <xsl:variable name="y2" as="xs:integer"
                    select="$yPoints[position() eq current() + 2]"/>
