...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--   default: 1                                              -->
    <!-- *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* -->
    <xsl:param name="thread_count" static="yes" as="xs:integer" select="1"/>
    ...
    <xsl:function name="djb:get_diag_cells" as="element(diag)+">
        ...
        <diag xsl:validation="preserve" xsl:default-validation="preserve">
            ...
            <xsl:for-each select="$row_start to $row_end" saxon:threads="{$thread_count}">
                <cell>
    ...
    <xsl:function name="djb:create_grid" as="element(diag)+">
        ...
        <xsl:for-each select="1 to $diag_count" saxon:threads="{$thread_count}" default-validation="preserve">
    ...
    <xsl:function name="djb:find_path" as="element(result)">
        ...
        <xsl:iterate select="1 to $diag_count" default-validation="preserve">
            ...
            <xsl:variable name="current" as="element(cell)+">
                <xsl:for-each select="$current_diag/cell" saxon:threads="{$thread_count}">
                    <!-- 
    ...
    <xsl:function name="djb:grid_to_html" as="element(html:table)">
        ...
        <table id="alignment_grid" xmlns="http://www.w3.org/1999/xhtml">
            ...
            <xsl:for-each select="distinct-values($cells/@row)" saxon:threads="{$thread_count}">
                <xsl:sort stable="no"/>
                ...
                <tr>
                    ...
                    <xsl:for-each select="$cells[@row = $row]" saxon:threads="{$thread_count}">
