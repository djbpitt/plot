<xsl:stylesheet version="3.0" xmlns="http://www.w3.org/1999/xhtml"
    xmlns:f="urn:stylesheet-functions" xmlns:xd="http://www.pnp-software.com/XSLTdoc"
    xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="f xd xhtml xs">
    <!-- https://github.com/jhellingman/tei2html/blob/master/levenshtein.xsl -->
    <xd:doc>
            ...
            select="
                if ($a = '') then
                    ...
                    if ($b = '') then
                        ...
                        f:calculate-levenshtein-distance(
    ...
    <xsl:function name="f:calculate-levenshtein-distance" as="xs:integer">
                    ...
                    select="
                        if ($i = 0) then
                            ...
                            if ($j = 0) then
                                ...
                                min(($lastDiag[$d - 1] + 1, $lastDiag[$d + 1] + 1,
                                $lastDiag[$d] + (if ($chars1[$i] eq $chars2[$j]) then
                                    0
                                else
                                    1)))"
                />
        ...
        <xsl:sequence
            select="
                if ($total = $length1 + $length2) then
                    exactly-one($diag)
                else
                    f:calculate-levenshtein-distance($chars1, $chars2, $length1, $length2, $diag, $total + 1)"
        />
    </xsl:function>
