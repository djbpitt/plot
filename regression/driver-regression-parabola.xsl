...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="maxX" as="xs:integer" select="500"/>
    ...
    <xsl:template name="xsl:initial-template">
        ...
        <xsl:variable name="parabola" as="element(svg:g)"
            select="djb:plot-parabolic-segment(50, 450, $a, $b, $c)"/>
        <xsl:if test="$debug">
        ...
        <svg xmlns="http://www.w3.org/2000/svg"
            viewBox="-10 -{$maxY + 10} {$maxX + 20} {$maxY + 20}">
            <defs>
                <clipPath id="clip">
                    <rect x="0" y="-{$maxY}" width="{$maxX}" height="{$maxY}"/>
                </clipPath>
            </defs>
            <style type="text/css">
                <![CDATA[
            ...
            <xsl:for-each select="0 to $maxY">
                ...
                <xsl:if test="$yPos mod 10 eq 0">
                    <line x1="0" y1="-{$yPos}" x2="{$maxX}" y2="-{$yPos}" stroke="lightgray"
                        stroke-width="0.5" stroke-linecap="square"/>
